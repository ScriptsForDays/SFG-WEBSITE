--[[
	FluxUI - Premium Customizable UI Library for Roblox Exploits
	Supports PC and Mobile platforms
	Highly customizable with editable components
]]
...
function FluxUI:Notify(title, message, duration)
	...
	msgLabel.Size = UDim2.new(1, -20, 0, 34)
	...
	pcall(function()
		...
		local props = {Size = container.Size}
		-- fade/slide not native for frames, so animate Position slightly
...
function FluxUI:CreateWindow(title, options)
	...
	local window = {}
	window.Title = title or "FluxUI Window"
	...
	local shadow = create("ImageLabel", {
		...
		-- stronger, less transparent shadow to match HTML preview
	...
	-- Drag Functionality (only from topbar)
	...
	local function setupDrag(topBarFrame, targetFrame)
		topBarFrame.InputBegan:Connect(function(input)
			...
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				dragging = true
				currentDraggingFrame = targetFrame
	...
	-- Setup dragging only from topbars (dragging topbar moves the main/compact frame)
	setupDrag(topBar, mainFrame)
	setupDrag(compactTopBar, compactFrame)
